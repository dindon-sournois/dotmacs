...
Use =<user-emacs-directory>/cache/= to store temporary data. This is to avoid
littering in the Emacs and project directories with an ever-growing number of
cache files (=.*~=, =.#*=, =.*.~undo-tree~=, etc.).
...
#+BEGIN_SRC emacs-lisp
(let ((auto-saves-dir (me/cache-concat "auto-saves/")))
...
(setq undo-tree-history-directory-alist `(("." . ,(me/cache-concat "undos"))))
#+END_SRC
