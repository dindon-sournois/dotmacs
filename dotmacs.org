...
#+BEGIN_SRC emacs-lisp
(setq-default
 load-prefer-newer t                      ; Don't reuse old byte-code
 ...
 compilation-scroll-output t)          ; Make sure the cursor goes to the end of log
...
;FIXME (with-eval-after-load 'url
...
;; (set-face-attribute 'default nil :height 120) ;; format: 1/X pt
(defun me//font-exists-p (font) (if (null (x-list-fonts font)) nil t))
(when (window-system)
  (cond ((me//font-exists-p "Dejavu Sans Mono for Powerline")
      (set-frame-font "Dejavu Sans Mono for Powerline:spacing=100:size=16" nil t))
        ))
;; ((me//font-exists-p "Courier New") (set-frame-font "Courier New:spacing=100:size=18" nil t))))
#+END_SRC
...
#+begin_src emacs-lisp
(define-globalized-minor-mode me/global-olivetti-mode olivetti-mode
  (lambda () (olivetti-mode 1)))

(me/global-olivetti-mode 0)
#+end_src
...
(advice-add 'helm-imenu :around #'me//evil-set-jump)
(advice-add 'helm-projectile-grep :around #'me//evil-set-jump)
...
(defun me/org-paste-image()
    (interactive)
    (org-download-clipboard (concat (read-string "filename: ") ".png")))

(with-eval-after-load 'org-download
  (evil-define-key 'normal org-mode-map
    (kbd "C-M-y") #'me/org-paste-image))
#+END_SRC

#+BEGIN_SRC emacs-lisp :tangle no

;; https://www.gnu.org/software/emacs/manual/html_node/elisp/Yanking-Media.html
...
same directory as the org-buffer and insert a link to this file."
  ...
  (let* ((target-file
          (concat
           ...
           (concat (read-string "filename: ") ".png")))
         (wsl-path
          (concat (me//as-windows-path(file-name-directory target-file))
                  "\\"
                  (file-name-nondirectory target-file)))
         (ps-script
          (concat "(Get-Clipboard -Format image).Save('" wsl-path "')")))

    (me//powershell ps-script)
...
(defun me//as-windows-path (unix-path)
...
;;   :config
;;   (setq guess-language-languages '(gb fr)
...
(me/leader-mode-def
  ...
  "gDv" '(:ignore t :which-key "find definitions other window vertical")
  "gDv" 'xref-find-definitions-other-window
  ;; "gDs" '(:ignore t :which-key "find definitions other window horizontal")
  ;; "gDs" 'me/xref-find-definitions-other-window-horizontal
...
(general-def ; FIXME: not working
  ...
  "M-h" 'markdown-table-move-column-left
  "M-l" 'markdown-table-move-column-right
  "M-k" 'markdown-table-move-row-up
  "M-j" 'markdown-table-move-row-down)
#+END_SRC
...
(me/leader-def
  ...
  "tCw" '(:ignore t :which-key "center-mode transient")
  "tCw" 'me/transient-center-width
  "tCc" '(:ignore t :which-key "center-mode")
  "tCc" 'olivetti-mode
  "tc" '(:ignore t :which-key "global-center-mode")
  "tc" 'me/global-olivetti-mode
  "tg" 'golden-ratio
...
#+BEGIN_SRC emacs-lisp
(defun me//transient-window-resize()(interactive))
(general-def
  :prefix-map 'transient-window-resize-map
  "q" '(:ignore t :which-key "quit")
  "q" 'me//transient-window-resize
  "[" '(:ignore t :which-key "shrink")
  "[" '(lambda()(interactive)(enlarge-window -3))
  "]" '(:ignore t :which-key "enlarge")
  "]" '(lambda()(interactive)(enlarge-window 3))
  "{" '(:ignore t :which-key "shrink horizontally")
  "{" '(lambda()(interactive)(enlarge-window-horizontally -3))
  "}" '(:ignore t :which-key "enlarge horizontally")
  "}" '(lambda()(interactive)(enlarge-window-horizontally 3))
  "h" '(:ignore t :which-key "left")
  "h" 'evil-window-left
  "l" '(:ignore t :which-key "right")
  "l" 'evil-window-right
  "k" '(:ignore t :which-key "up")
  "k" 'evil-window-up
  "j" '(:ignore t :which-key "down")
  "j" 'evil-window-down)
(hercules-def
 :toggle-funs 'me//transient-window-resize
 :keymap 'transient-window-resize-map
 :transient t)
#+END_SRC
...
(me/leader-def
  ...
  "w."     '(:ignore t :which-key "resize")
  "w."     'me//transient-window-resize)
#+END_SRC
